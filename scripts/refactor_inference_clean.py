...
# We replace everything from "class PinnInference:" down to "def run_full_simulation"
start_class = "class PinnInference:"
end_method = '    def run_full_simulation(self, payload: Dict[str, Any]) -> Dict[str, Any]:'

head = content[:content.find(start_class)]
tail = content[content.find(end_method) + len(end_method):]

# Now inside tail, we find "bu_raw = self.predict_yield_bu_acre(payload)" and replace it
tail = tail.replace(
    'bu_raw = self.predict_yield_bu_acre(payload)',
    'bu_raw = self.adapter.predict_yield_bu_acre(payload)\n        is_fallback = False'
)

# And "inference_mode = "pinn" if (bu_raw is not None and self.available) else "pinn-calibrated-surrogate""
tail = tail.replace(
...
# Now find the final return dict
return_start = '        return {\n            "id":'
return_replacement = """        interval = self.adapter.get_prediction_interval(payload, bu_raw) if not is_fallback else {"lower_kg_ha": projected_yield * 0.85, "upper_kg_ha": projected_yield * 1.15}
...
# Replace the global instances
tail = tail.replace('_inference = PinnInference()', '_inference = YieldInferenceService()')
tail = tail.replace('def get_inference() -> PinnInference:', 'def get_inference() -> YieldInferenceService:')


with open(file_path, "w", encoding="utf-8") as f:
    f.write(head + new_classes + tail)
print("Updated inference.py perfectly!")
