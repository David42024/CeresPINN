        ...
        <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 flex flex-col justify-between space-y-4">
          <div>
            <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-800">
              <span className="px-2.5 py-0.5 rounded-lg bg-slate-200 dark:bg-slate-800 text-slate-800 dark:text-slate-200 font-bold text-xs">
                {t('whatIfStudio.scenarioABaseline')}
              </span>
            ...
            <div className="space-y-3 mt-3 text-xs">
              <div>
                <label className="block text-slate-600 dark:text-slate-400 mb-1">{t('whatIfStudio.irrigationStrategyLabel')}</label>
                <select
                ...
                >
                  <option value="rainfed">{t('whatIfStudio.optRainfed')}</option>
                  <option value="deficit_50">{t('whatIfStudio.optDeficit50')}</option>
                  <option value="optimal_100">{t('whatIfStudio.optOptimal100')}</option>
                  <option value="smart_sensor">{t('whatIfStudio.optSmartSensor')}</option>
                </select>
              </div>

              <div>
                <label className="block text-slate-600 dark:text-slate-400 mb-1">{t('whatIfStudio.maizeVarietyLabel')}</label>
                <select
                  value={configA.maizeVariety}
                  onChange={(e) => handleUpdateConfigA({ maizeVariety: e.target.value as MaizeVariety })}
                  className="w-full px-2.5 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-slate-200"
                >
                  <option value="short_cycle">{t('whatIfStudio.optShortCycle')}</option>
                  <option value="medium_cycle">{t('whatIfStudio.optMediumCycle')}</option>
                  <option value="long_cycle">{t('whatIfStudio.optLongCycle')}</option>
                </select>
              </div>

              <div>
                <label className="block text-slate-600 dark:text-slate-400 mb-1">{t('whatIfStudio.climateScenarioLabel')}</label>
                <select
          ...
          <div className="p-3.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-2 text-xs">
            <div className="flex justify-between">
              <span className="text-slate-600 dark:text-slate-400">{t('whatIfStudio.yieldLabel')}</span>
            ...
            <div className="flex justify-between">
              <span className="text-slate-600 dark:text-slate-400">{t('whatIfStudio.waterConsumedLabel')}</span>
              <strong className="text-cyan-700 dark:text-cyan-300 font-mono">{resultA.summaryKPIs.totalWaterConsumedMm} mm</strong>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-600 dark:text-slate-400">{t('whatIfStudio.cwsiMaxLabel')}</span>
              <strong className="text-amber-600 dark:text-amber-400 font-mono">{resultA.summaryKPIs.peakWaterStressIndex.toFixed(2)}</strong>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-600 dark:text-slate-400">{t('whatIfStudio.marginLabel')}</span>
          ...
          <div>
            <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-800">
              <span className="px-2.5 py-0.5 rounded-lg bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800 font-bold text-xs">
                {t('whatIfStudio.scenarioCSensorRes')}
              </span>
