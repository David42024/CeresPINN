...
export const DataPipelinesView: React.FC = () => {
  ...
  return (
    <div id="data-pipelines-view" className="bg-white dark:bg-slate-900/90 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-xl space-y-5">
      {/* Header */}
        ...
        <div className="flex items-center gap-2">
          <span className={`flex items-center gap-1.5 px-3 py-1 rounded-xl text-xs font-mono border ${
            backendAvailable === false
              ? 'bg-amber-50 dark:bg-amber-950/80 border-amber-200 dark:border-amber-800/60 text-amber-800 dark:text-amber-300'
              : 'bg-emerald-50 dark:bg-emerald-950/80 border-emerald-200 dark:border-emerald-800/60 text-emerald-800 dark:text-emerald-300'
          }`}>
            <Radio className={`w-3.5 h-3.5 ${backendAvailable === false ? 'text-amber-500 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400 animate-pulse'}`} />
            {backendAvailable === false
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {pipelines.map((pipe) => {
          ...
          return (
            <div
              key={pipe.id}
              className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 flex flex-col justify-between space-y-4 hover:border-slate-400 dark:hover:border-slate-700 transition-all"
            >
              <div>
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-2.5">
                    <div className="p-2 rounded-xl bg-white dark:bg-slate-900 text-emerald-600 dark:text-emerald-400 border border-slate-200 dark:border-slate-800">
                      {pipe.id === 'pipe-chirps' ? <CloudRain className="w-5 h-5 text-cyan-600 dark:text-cyan-400" /> :
                       pipe.id === 'pipe-nasa-nex' ? <Satellite className="w-5 h-5 text-amber-500 dark:text-amber-400" /> :
                       <Globe className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />}
                    </div>
                    <div>
                      <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 leading-tight">{pipe.name}</h3>
                      <span className="text-[11px] text-slate-600 dark:text-slate-400 font-mono">{pipe.frequency}</span>
                    </div>
                  </div>
                ...
                <p className="text-xs text-slate-700 dark:text-slate-300 mt-3 leading-relaxed">{pipe.description}</p>
              ...
              <div className="space-y-1.5">
                ...
                <button
                  ...
                  disabled={isSyncing}
                  className="w-full py-2 rounded-xl bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-800 dark:text-slate-200 font-semibold text-xs flex items-center justify-center gap-1.5 border border-slate-300 dark:border-slate-700 transition-all disabled:opacity-50"
                >
